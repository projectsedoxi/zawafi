...
mainNavigationTrigger(){
  ...
  this.mainTrigger.addEventListener("click", (e)=>{
          ...
          if(window.innerWidth>=this.breakPoint) {
              ...
              e.currentTarget.classList.toggle('main-nav__trigger--active');
              let mainContent = e.currentTarget.nextElementSibling;
              slideToggle(mainContent)
          }
      });
...
submenuDetect(){
  ...
  li.forEach((item)=>{
      ...
      if(ul){
          ...
          item.classList.add('submenu-trigger');
          ...
          if(window.innerWidth<=this.breakPoint) {
              ...
              item.querySelector('.main-nav__link').appendChild(iconElement)
          }
      }
  });
...
firstLevelMenuToggle(){
  let firstLevelMenu = this.mainContent.querySelector("ul").querySelectorAll(":scope > li.has-submenu");
  firstLevelMenu.forEach((item)=>{
      if(window.innerWidth<=this.breakPoint) {
          let submenuTrigger = item.querySelector(":scope > .down-arrow-icon");
      ...
      hasSubmenu.forEach((item)=>{
        ...
        ul.forEach((item)=>{
          item.classList.add('main-nav__submenu-content');
        })
      })
...
document.body.classList.remove('no-js','singular', 'is-light-theme');
...
/* Blog Detail Active Class Menu */

let menulinks=document.querySelectorAll('.main-nav__ul li a');
let blogdetailpage=document.querySelector('.blog-detail-page');
let servicepage=document.querySelector('.service-detail-page');
let blogrelatedpages=document.querySelector('.blog-list-container');
if(blogdetailpage || blogrelatedpages) {
  let checkstring=menulinks.forEach((item)=>{
    if(item.innerText==="Blogs") {
      console.log("Hiiiii");
      console.log(item.parentElement)
      item.parentElement.classList.add("main-nav__item--active");
    };
  })
}
