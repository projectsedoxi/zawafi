/* global zawafiIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'zawafi-supports-dark-theme' ) ) {
	zawafiDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function zawafiDarkModeEditorInit( attempt ) {
	...
	if ( zawafiIsDarkMode() ) {
		if ( null === container ) {
			...
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						zawafiDarkModeEditorInit( attempt + 1 );
					},
