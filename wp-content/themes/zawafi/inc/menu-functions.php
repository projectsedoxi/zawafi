...
function twenty_twenty_one_add_sub_menu_toggle( $output, $item, $depth, $args ) {
	if ( 0 === $depth && in_array( 'menu-item-has-children', $item->classes, true ) ) {
		...
		// Add toggle button.
		$output .= '<button class="sub-menu-toggle" aria-expanded="false" onClick="zawafiExpandSubMenu(this)">';
		...
		$output .= '<span class="screen-reader-text">' . esc_html__( 'Open menu', 'zawafi' ) . '</span>';
		$output .= '</button>';
	}
	return $output;
}
...
function twenty_twenty_one_get_social_link_svg( $uri, $size = 24 ) {
	return zawafi_SVG_Icons::get_social_link_svg( $uri, $size );
}
